...
static char	get_type(t_symtab *tab)
{
	t_index	*index;
	char	ret;
	uint8_t	type;
...
void		print_symtab(t_symtab *tab, size_t size, int width)
{
	uint8_t	type;
	size_t	i;
	...
	while (i < size)
	{
		...
		if (type == N_UNDF)
			...
			ft_printf("%*c %c %s (indirect for %s)\n", width, ' ',
				get_type(tab), tab->n_name, tab->n_name);
		else
			ft_printf("%s %c %s\n", uint64_to_hex(tab->n_value, width),
				get_type(tab), tab->n_name);
