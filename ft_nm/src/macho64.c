...
static void	read_symtab(struct symtab_command *symcmd, t_symtab *tab, void *ptr)
{
...
int			read_symtab_macho64(void *ptr, t_symtab **symtab, uint32_t *size)
{
	struct symtab_command *symtab_lc;

	*symtab = NULL;
	symtab_lc = get_symtab_lc(ptr);
	*symtab = malloc(sizeof(t_symtab) * symtab_lc->nsyms);
	if (symtab == NULL)
		return (1);
